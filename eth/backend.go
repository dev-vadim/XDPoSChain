...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	eth.miner = miner.New(eth, &config.Miner, chainConfig, eth.EventMux(), eth.engine, eth.isLocalBlock, ctx.GetConfig().AnnounceTxs)
...
func (s *Ethereum) ValidateMasternode() (bool, error) {
	...
	if s.blockchain.Config().XDPoS != nil {
...
func (s *Ethereum) StartStaking(threads int) error {
	...
	if !s.IsStaking() {
		...
		if XDPoS, ok := s.engine.(*XDPoS.XDPoS); ok {
			...
			XDPoS.Authorize(eb, wallet.SignData)
		}
...
// StopStaking terminates the miner, both at the consensus engine level as well as
