...
func main() {
	flag.Parse()
	lddb, _ := rawdb.NewLevelDBDatabase(*dir, eth.DefaultConfig.DatabaseCache, utils.MakeDatabaseHandles(), "")
...
func findNewNodes(nodes []*StateNode, db ethdb.Database, batchlvdb ethdb.Batch) ([][17]*StateNode, int) {
	length := len(nodes)
